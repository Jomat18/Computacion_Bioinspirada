...
def actualizar_feromona(rutas, fitness_hormigas, menor_costo, ruta_menor,menor):
	menor=menor.tolist()
	size=len(distancias)
	sumas=[0.0]
	agregar=True
	...
	for i in range(size-1):
		for j in range(i+1,size):
			inicio=ciudades[i]
			fin=ciudades[j]
			suma_delta=feromona[ord(inicio)%65][ord(fin)%65]
			 	...
			 	for r in range(len(rutas[k])-1):
					if inicio==rutas[k][r] or fin==rutas[k][r]:
						if  inicio==rutas[k][r+1] or fin==rutas[k][r+1]:
							temp=(w-(menor.index(k)+1))*(1.0/fitness_hormigas[k])
							agregar=False
							sumas.append(temp)
							break
			...
			if encontrar_nodo(inicio,fin,ruta_menor):
				mejor_global=w*(1.0/menor_costo)
			else:
				mejor_global=0.0
...
def generaciones():
	...
	for m in range(n_iteraciones):
		...
		actualizar_feromona(rutas,fitness_hormigas,fitness_hormigas[menor[0]], rutas[menor[0]],menor)
