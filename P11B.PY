...
def poblacion_agregada(pob, size_, desv):
		
	pob_copy=copy.copy(pob)

	for i in range(size_):
		p1,p2,d1,d2 = torneo(pob_copy,desv)
		hijo, desvh = cruzamiento(p1,p2,d1,d2)
		hijo, desvh = mutacion(hijo, desvh)	
		pob=np.append(pob, [hijo], axis=0)
		desv=np.append(desv, [desvh], axis=0)
...
def determinista(pob, desvp):

	for i in range(size_):
		vector=vector_fitness(pob, len(pob))
		vector_copy=copy.copy(vector)
		indice = np.argsort(vector_copy)							
		pob = np.delete(pob,indice[len(indice)-1],axis=0)
		desvp = np.delete(desvp,indice[len(indice)-1],axis=0)
...
def generaciones():
	...
	for i in range(100):
		...
		print("*******************Poblacion***************************")
		print(pob)	
		pob,desv = poblacion_agregada(pob, size_, desv) 
		pob, desv = determinista(pob, desv)		
		print("*******************Poblacion nueva***************************")
		print(pob)
		print("*******************Desviacion***************************")
		print(desv)
		print("*******************Fitness***************************")
		print(vector_fitness(pob,size))
